	$(document).ready(function(){
		 ...
		 $(".base-opt-btn").on("click",".base-opt-btn-item",function(){
		    ...
		    $("#ods-close").click(function () {
   	      ...
   	      var totalnum=0;//总条数
   	      var count=0;//分页总条数
   	      var queryOdsInterNum = function(dispatcName,dataSouceId,isDay,dispatcType,status){
		 ...
		 var refresh=function(dataSouceId,page,status){
			  isDay = $("#select_button1 option:checked").val();
			  dispatcType = $("#select_button2 option:checked").val();
			  dispatcName = $("#dispatcName").val();
	   	      	  ...
	   	      	  success:function(data){
	   	      	    ...
	   	      	    $.each(data,function(k,v){
	   	      	        ...
	   	      	        if(totalnum==0){
		   	      	        ...
		   	      	        $trTmp.append($td.clone().text(v.flowName).attr("title",v.errorDec));
		   	      	        /**预警原因**/
		   	      	        var msg="";
						     if(v.status == "1" || v.status=="2"){
						    	...
						    	if(v.errorMsg==""||v.errorMsg=="文件未到"){
		   	      	        ...
		   	      	        $trTmp.append($td.clone().text(v.nodeName==undefined||v.nodeName==""?"":v.nodeName));
		   	      	        $trTmp.append($td.clone().text(v.beginDate));
		   	         	    $trTmp.append($td.clone().text(v.actualDate));
		   	         	    $trTmp.append($td.clone().text(v.demainDate));
		   	         	    $trTmp.append($td.clone().text(v.updateTime));
			   	      	   var $tdLast=$td.clone();
	     	        		$($tdLast).bind("click",function(){
	     	        			$(".foundaods").show();
	     	        			$(".status").html(Header.setOdsStatus(v.status,v.timeOut));
	     	        			if(v.status==0){
	     	        				$(".result").html("加载成功");
	     	        			}else if(v.status==-1){
	     	        				$(".result").html("加载失败");
	     	        			...
	     	        			$(".flowName").html(v.flowName);
	     	        			$(".detail").html(v.errorDec);
	     	        			$(".beginDate").html(v.beginDate);
	     	        			$(".nodeName").html(v.nodeName);
